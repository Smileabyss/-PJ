...
def train(i, data, path):
    ...
    if os.path.exists(path):
        ...
        try:
                ...
                losses = checkpoint['loss']  # 用于记录损失值
                print("模型成功加载")
    ...
    with tqdm(total=args.epochs-epoch_init_, desc="Training", unit="step") as pbar:
        for epoch in range(epoch_init_, args.epochs):
            ...
            input_ = torch.tensor(np.array(data[ train_data_index ]))
        

            # print(vis.shape, ir.shape, gt.shape)
            # 在每次计算损失之前，将数据传送到GPU
            img = input_.to(device)
            img = img.float()
            ...
            # resolution loss
            gt = Variable(img.data.clone(), requires_grad=False)
...
if __name__ == "__main__":
    data = np.load(args.data_vstack_path)
    # print(data.shape)
    # 设置随机数种子
