...
def spatial_fusion(tensor1, tensor2, spatial_type='mean'):
    ...
    spatial_w1 = spatial_w1.repeat( shape[0], 1, 1)
    spatial_w2 = spatial_w2.repeat( shape[0], 1, 1)
...
def spatial_attention(tensor, spatial_type='mean'):
    if spatial_type == 'mean':
        spatial = tensor.mean(dim=0, keepdim=True)
    elif spatial_type == 'sum':
        spatial = tensor.sum(dim=0, keepdim=True)
    return spatial
