...
def train(i, data, path):
    ...
    # 将模型移动到GPU上
    model = Net.DenseFuse_net(args.in_channels[0],args.in_channels[0], args.height, args.width, args.hidden_sizes)
    # model.to(device)
    ...
    with tqdm(total=args.epochs-epoch_init_, desc="Training", unit="step") as pbar:
        for epoch in range(epoch_init_, args.epochs):
            ...
            # 随机抽取数据改为轮流抽取数据训练
            start = epoch % 300 * args.batch_size
            train_data_index = range(start,start + args.batch_size)
            # 将 NumPy 数组转换为 PyTorch 张量
            vis = torch.tensor(np.array([data[i][0] for i in train_data_index]))/255.0
            ir = torch.tensor(np.array([data[i][1] for i in train_data_index]))/255.0
            ...
            # encoder
            en0 = model.encoder(vis.float())
            en1 = model.encoder(ir.float())

            # defense alyers
            en = model.fusion_net(en0, en1)
            # decoder
            outputs = model.decoder(en)*255
            # resolution loss
